...
export const nameCode = 'Bucket' // 请不要修改
...
export const getRoutes = (parentPath: string) => {
  ...
  return [
    {
      path: `${nameCode}`,
      component: () => import('@/views/storageManagement/Bucket/BucketList.vue'),
      name: `${nameCode}List`,
      meta: {
        title
      },
    ...
    {
      path: `${nameCode}Edit`,
      component: () => import('@/views/storageManagement/Bucket/BucketEdit.vue'),
      name: `${nameCode}Edit`,
      meta: {
        title,
        noTagsView: true,
